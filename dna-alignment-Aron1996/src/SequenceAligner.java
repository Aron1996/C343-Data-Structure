...
public class SequenceAligner {
  ...
  private void fillCache() {
      ...
      for (int i = 1; i < cache[0].length; i++) {
          this.cache[0][i] = new Result(i * judge.getGapCost(), Direction.LEFT);
      }
      for (int i = 1; i < cache.length; i++) {
          this.cache[i][0] = new Result(i * judge.getGapCost(), Direction.UP);
      }
      for (int i = 1; i < cache.length; i++) {
          for (int j = 1; j < cache[i].length; j++) {
              int M = cache[i - 1][j - 1].getScore() + judge.score(x.charAt(i - 1), y.charAt(j - 1));
              int D  = cache[i - 1][j].getScore() + judge.getGapCost();
              int I = cache[i][j - 1].getScore() + judge.getGapCost();
              int max = Math.max(M, Math.max(D, I));
              if(max == M){
                  cache[i][j] = new Result(max, Direction.DIAGONAL);
              }
              else if(max == I){
                  cache[i][j] = new Result(max, Direction.LEFT);
              }else{
                  cache[i][j] = new Result(max, Direction.UP);
              }
          }
      }
  }
  ...
  private void traceback() {
      ...
      alignedY = "";
      cache[n][m].markPath();
      while (!cache[n][m].getParent().equals(Direction.NONE)) {

          if (cache[n][m].getParent().equals(Direction.DIAGONAL)) {
              alignedX = x.charAt(--n) + alignedX;
              alignedY = y.charAt(--m) + alignedY;
          } else if (cache[n][m].getParent().equals(Direction.LEFT)) {
