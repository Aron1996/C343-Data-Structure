...
class BinomialHeap<K extends Comparable<K>> {
    ...
    * @precondition this.height == other.height
     ...
     */
    boolean isHeap() {
        return isHeap(this,children);
    }
    boolean isHeap(BinomialHeap<K> bh, ListNode<BinomialHeap<K>> lb){
        boolean result;
        if(bh == null){
            return true;
        }
            ...
            if(bh.key.compareTo(lb.data.key) <= 0){
                result = true;
                isHeap(lb.data, lb.next);
            }else{
                result = false;
            }
        }
...
public class BinomialQueue<K extends Comparable<K>> {
    ...
    insert(BinomialHeap<K> n, ListNode<BinomialHeap<K>> ns) {
        if(ns == null){
            ...
            return new ListNode<>(n , ns);
        }else if(n.height == ns.data.height){
            BinomialHeap<K> newHeap = ns.data.link(n);
            return insert(newHeap, ns.next);
        }else{
    ...
    ListNode<BinomialHeap<K>> merge(ListNode<BinomialHeap<K>> ns1, ListNode<BinomialHeap<K>> ns2) {
	    if(ns1 == null){
	        ...
	        ListNode<BinomialHeap<K>> result;
	        ListNode<BinomialHeap<K>> temp = ns1.next;
	        result = insert(ns1.data, ns2);
            while(temp != null){
                result = insert(temp.data, result);
                temp = temp.next;
            }
            return result;
        }
